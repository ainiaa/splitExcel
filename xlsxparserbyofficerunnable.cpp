...
XlsxParserByOfficeRunnable::XlsxParserByOfficeRunnable(QObject *parent)
{
    this->mParent = parent;
}

XlsxParserByOfficeRunnable::~XlsxParserByOfficeRunnable()
{
    runnableID = 0;
}

void XlsxParserByOfficeRunnable::setID(const int &id)
{
    runnableID = id;
}

void XlsxParserByOfficeRunnable::setSplitData(QString sourcePath, QString selectedSheetName, QHash<QString, QList<int>> fragmentDataQhash, QString savePath, int total)
{
...
void XlsxParserByOfficeRunnable::run()
{
...
void XlsxParserByOfficeRunnable::processByOffice(QString key, QList<int> contentList)
{
    QString xlsName;
...
void XlsxParserByOfficeRunnable::requestMsg(const int msgType, const QString &msg)
{
    qobject_cast<XlsxParserByOffice *>(mParent)->receiveMessage(msgType,msg.arg(msgType));
...
///
void XlsxParserByOfficeRunnable::convertToColName(int data, QString &res)
{
...
/// \return
///
QString XlsxParserByOfficeRunnable::to26AlphabetString(int data)
{
    QChar ch = data + 0x40;//A对应0x41
    return QString(ch);
}

bool XlsxParserByOfficeRunnable::copyFileToPath(QString sourceDir ,QString toDir, bool coverFileIfExist)
{
...
void XlsxParserByOfficeRunnable::processSourceFile()
{
    QAxObject *excel = new QAxObject("Excel.Application");//连接Excel控件
    excel->dynamicCall("SetVisible (bool Visible)",false);//不显示窗体
    excel->setProperty("DisplayAlerts",  false);//不显示任何警告信息。如果为true那么在关闭是会出现类似“文件已修改，是否保存”的提示
    ...
    qDebug() << " sourceRowStart:" <<sourceRowStart<< " sourceColStart:" <<sourceColStart<< " sourceRowCnt:" <<sourceRowCnt<< " sourceColCnt:" <<sourceColCnt << " sourceMinAlphabetCol:" <<sourceMinAlphabetCol << " sourceMaxAlphabetCol:" << sourceMaxAlphabetCol;

    this->generateTplXls();//生成模板文件
}

void XlsxParserByOfficeRunnable::generateTplXls()
{
    QString xlsName;
