...
/**
 * @brief SplitOnlyWindow::on_splitOnlySubmitPushButton_clicked
 */
void SplitOnlyWindow::on_splitOnlySubmitPushButton_clicked() {
    ...
    if (ui->xlsObjLineEdit->text().isEmpty()) {
        QMessageBox::information(this, "Save Path Error", "请选择待拆分的excel文件(*.xlsx)");
        ui->splitOnlySubmitPushButton->setDisabled(false);
        return;
    }
    if (savePath.length() == 0) {
        QMessageBox::information(this, "Save Path Error", "请选择新文件的保存路径");
        ui->splitOnlySubmitPushButton->setDisabled(false);
        return;
    }
...
/**
 *
 * @brief SplitOnlyWindow::doSplitXls 拆分
 * @param dataSheetName 数据sheet的名字
 * @param savePath 新文件保持路径
 */
